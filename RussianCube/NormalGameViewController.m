...
#import "CubeF.h"
#import "CubeG.h"
#import "NormalGameMenuTableViewController.h"
...
//菜单中tableView的控制类
@property (nonatomic,strong)NormalGameMenuTableViewController *gameMenuTVC;
...
- (void)viewDidLoad {
    ...
    //初始化菜单控制类
    self.gameMenuTVC = [[NormalGameMenuTableViewController alloc] init];
...
- (void)initUI {
    ...
    //俄罗斯方块的主体盒子
    UIImageView *cubeBox = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"cubeBox2.png"]];
    ...
    [gameMenu setImage:[UIImage imageNamed:@"gameMenuSelected.png"] forState:UIControlStateSelected];
    ...
    [gameLevelLabel setTextColor:[UIColor whiteColor]];
    ...
    [self.gameLevelLabelValue setTextColor:[UIColor whiteColor]];
    ...
    [gameScoreLabel setTextColor:[UIColor whiteColor]];
    ...
    [self.gameScoreLabelValue setTextColor:[UIColor whiteColor]];
    ...
    [gameScoreRecordLabel setTextColor:[UIColor whiteColor]];
    ...
    [self.gameScoreRecordLabelValue setTextColor:[UIColor whiteColor]];
...
- (void)checkScore {
    ...
    if (lineCount != 0) {
        long getScore = (2*lineCount-1)*100;
...
- (void)died {
    [self showAlertMessage:@"施主 走好 不送 再来否？" ifYes:^{[self restartGame];} ifNO:^{[self returnToLaunchView];}];
}
...
- (void) returnToLaunchView {
    [self dismissViewControllerAnimated:NO completion:^{
        [[NSNotificationCenter defaultCenter] postNotificationName:@"resumeLaunchView" object:self];
    }];
...
- (void)restartGameForMenu {
    [self showAlertMessage:@"确认重新开始？" ifYes:^{[self closeGameMenu];[self restartGame];} ifNO:^{[self closeGameMenu];}];
}
...
- (void)returnToLaunchViewForMenu {
    [self showAlertMessage:@"确认返回主菜单？" ifYes:^{[self returnToLaunchView];} ifNO:^{[self closeGameMenu];}];
}
